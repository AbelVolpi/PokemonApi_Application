...
class PokemonRepositoryImplTest {
    ...
    private fun createMockPokemonListResponse() =
        PokemonListResponse(
            results = listOf(
    ...
    private fun createMockDetailedPokemonResponse(pokemonName: String) =
        DetailedPokemonResponse(
            name = pokemonName,
            typeResponses = listOf(
                TypeResponse(
                    SubTypeResponse(
                        typeName = "electric"
                    )
            ...
            stats = listOf(
                StatItemResponse(
                    baseStat = 35,
                    statResponse = StatResponse(name = "hp")
                ),
                StatItemResponse(
                    baseStat = 55,
                    statResponse = StatResponse(name = "attack")
                )
            )
        )

    private fun createExpectedDetailedPokemon(pokemonName: String) =
        DetailedPokemon(
            name = pokemonName,
            types = listOf(
                Type(
